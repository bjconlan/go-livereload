/**
 * A simple implementation of the livereload protocol
 *
...
func websocketSubscriptionRegistry() {
	connections := make(map[*websocket.Conn]int)
	for {
		select {
			case subscription := <-subscriptionChannel:
				if subscription.active {
					connections[subscription.websocket] = 0
				} else {
					delete(connections, subscription.websocket)
				}
...
func liveReload16ClientHandler(ws *websocket.Conn) {
	defer func() {
		subscriptionChannel <- subscription{ws, false}
		fmt.Printf("Disconnected")
		ws.Close()
	}()

	subscriptionChannel <- subscription{ws, true}
	// on open
	for {
		var message string
...
func websocketHandshakeHandler(w http.ResponseWriter, req *http.Request) {
	websocket.Handler(liveReload16ClientHandler).ServeHTTP(w, req)
}

func liveReloadJavascriptFileHandler(w http.ResponseWriter, req *http.Request) {
	w.Header().Set("Content-Type", "text/javascript")
	io.WriteString(w, livereloadJavascriptString)
}

func Start(hostname string, port uint) {
	go websocketSubscriptionRegistry()
	http.HandleFunc("/websocket", websocketHandshakeHandler)
	http.HandleFunc("/livereload.js", liveReloadJavascriptFileHandler)
...
// https://raw.github.com/livereload/livereload-js/master/dist/livereload.js
const livereloadJavascriptString = `(function() {
